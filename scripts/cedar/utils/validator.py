...
def validate_template(api_key, document, request_url=None):
    if request_url is None:
        request_url = "https://resource.metadatacenter.orgx/command/validate?resource_types=template"
    return send_post_request(api_key, request_url, document)


def validate_instance(api_key, document, request_url=None):
    if request_url is None:
        request_url = "https://resource.metadatacenter.orgx/command/validate?resource_types=instance"
    return send_post_request(api_key, request_url, document)


def validate_element(api_key, document, request_url=None):
    if request_url is None:
        request_url = "https://resource.metadatacenter.orgx/command/validate?resource_types=element"
    return send_post_request(api_key, request_url, document)
